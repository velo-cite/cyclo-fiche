	...
	if (isset($_SESSION['user'])) {
		switch (SGBD) {
			case 'mysql':
				...
				$sql = "SELECT *, commune.lib_commune, st_x(poi.geom_poi) AS X, st_y(poi.geom_poi) AS Y, subcategory.icon_subcategory FROM poi INNER JOIN subcategory ON (subcategory.id_subcategory = poi.subcategory_id_subcategory) INNER JOIN commune ON (commune.id_commune = poi.commune_id_commune) INNER JOIN priorite ON (poi.priorite_id_priorite = priorite.id_priorite) WHERE ";
				$whereClause = '';
				if (isset($_GET['id'])) {
