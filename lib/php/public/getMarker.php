<?php header('Content-Type: text/html; charset=UTF-8');
	...
	switch (SGBD) {
		case 'mysql':
			...
			if (isset($_GET['id'])) {
				$sql = "SELECT *, commune.lib_commune, st_x(poi.geom_poi) AS X, st_y(poi.geom_poi) AS Y, subcategory.icon_subcategory, subcategory.lib_subcategory FROM poi INNER JOIN subcategory ON (subcategory.id_subcategory = poi.subcategory_id_subcategory) INNER JOIN commune ON (commune.id_commune = poi.commune_id_commune) INNER JOIN priorite ON (poi.priorite_id_priorite = priorite.id_priorite) WHERE poi.moderation_poi = TRUE AND delete_poi = FALSE AND poi.id_poi = ".$_GET['id'];
			} else {
				...
				$sql = "SELECT *, commune.lib_commune, st_x(poi.geom_poi) AS X, st_y(poi.geom_poi) AS Y, subcategory.icon_subcategory, subcategory.lib_subcategory FROM poi INNER JOIN subcategory ON (subcategory.id_subcategory = poi.subcategory_id_subcategory) INNER JOIN commune ON (commune.id_commune = poi.commune_id_commune) INNER JOIN priorite ON (poi.priorite_id_priorite = priorite.id_priorite) INNER JOIN status ON (status.id_status = poi.status_id_status) ";
				$sql .= $sqlappend;
				$sql .= $datesqlappend;
				$sql .= $statussqlappend;
			}
