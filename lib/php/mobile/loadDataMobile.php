<?php
	...
	switch (SGBD) {
		case 'mysql':
			...
			$sql = "SELECT id_commune, st_asText(geom_commune) AS geom FROM commune";
			$result = mysql_query($sql);
			while ($row = mysql_fetch_array($result)) {
				$commune = $row['geom'];
			...
			$pole_id_pole = 9;
			$sql = "SELECT id_pole, st_asText(geom_pole) AS geom FROM pole";
			$result = mysql_query($sql);
			while ($row = mysql_fetch_array($result)) {
				$pole = $row['geom'];
